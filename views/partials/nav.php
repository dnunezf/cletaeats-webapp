<nav class="nav-drawer" id="navDrawer">
    <div class="nav-brand">
        <div class="nav-brand-icon">
            <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path d="M11 9H9V2H7v7H5V2H3v7c0 2.12 1.66 3.84 3.75 3.97V22h2.5v-9.03C11.34 12.84 13 11.12 13 9V2h-2v7zm5-3v8h2.5v8H21V2c-2.76 0-5 2.24-5 4z"/>
            </svg>
        </div>
        <span class="nav-brand-text"><?= e(APP_NAME) ?></span>
    </div>

    <div class="nav-menu">
        <div class="nav-section-title">Main</div>

        <a href="<?= baseUrl('dashboard') ?>" class="nav-item <?= ($currentPage ?? '') === 'dashboard' ? 'active' : '' ?>">
            <svg viewBox="0 0 24 24"><path d="M3 13h8V3H3v10zm0 8h8v-6H3v6zm10 0h8V11h-8v10zm0-18v6h8V3h-8z"/></svg>
            <span>Dashboard</span>
        </a>

        <?php if (userIsAdmin()): ?>
            <div class="nav-section-title">Management</div>

            <a href="<?= baseUrl('customers') ?>" class="nav-item <?= ($currentPage ?? '') === 'customers' ? 'active' : '' ?>">
                <svg viewBox="0 0 24 24"><path d="M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5c-1.66 0-3 1.34-3 3s1.34 3 3 3zm-8 0c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5C6.34 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5c0-2.33-4.67-3.5-7-3.5zm8 0c-.29 0-.62.02-.97.05 1.16.84 1.97 1.97 1.97 3.45V19h6v-2.5c0-2.33-4.67-3.5-7-3.5z"/></svg>
                <span>Customers</span>
            </a>

            <a href="<?= baseUrl('restaurants') ?>" class="nav-item <?= ($currentPage ?? '') === 'restaurants' ? 'active' : '' ?>">
                <svg viewBox="0 0 24 24"><path d="M11 9H9V2H7v7H5V2H3v7c0 2.12 1.66 3.84 3.75 3.97V22h2.5v-9.03C11.34 12.84 13 11.12 13 9V2h-2v7zm5-3v8h2.5v8H21V2c-2.76 0-5 2.24-5 4z"/></svg>
                <span>Restaurants</span>
            </a>

            <a href="<?= baseUrl('drivers') ?>" class="nav-item <?= ($currentPage ?? '') === 'drivers' ? 'active' : '' ?>">
                <svg viewBox="0 0 24 24"><path d="M18 18.5c.83 0 1.5-.67 1.5-1.5s-.67-1.5-1.5-1.5-1.5.67-1.5 1.5.67 1.5 1.5 1.5zM19.5 9.5h-1.84l-1.48-4.45C15.92 4.42 15.33 4 14.66 4H12v2h2.65l1.67 5H5.5c-1.38 0-2.5 1.12-2.5 2.5v3.5h2c0 1.66 1.34 3 3 3s3-1.34 3-3h4c0 1.66 1.34 3 3 3s3-1.34 3-3h2V14c0-2.76-2.24-4.5-4.5-4.5zM8 18.5c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zM13 9H5c-.55 0-1 .45-1 1s.45 1 1 1h8V9z"/></svg>
                <span>Delivery Drivers</span>
            </a>
        <?php endif; ?>

        <?php if (userIsAnyOf(['admin', 'restaurant'])): ?>
            <?php if (!userIsAdmin()): ?>
                <div class="nav-section-title">Management</div>
            <?php endif; ?>
            <a href="<?= baseUrl('combos') ?>" class="nav-item <?= ($currentPage ?? '') === 'combos' ? 'active' : '' ?>">
                <svg viewBox="0 0 24 24"><path d="M3 3h18v2H3V3zm0 4h18v2H3V7zm0 4h12v2H3v-2zm0 4h12v2H3v-2zm0 4h18v2H3v-2z"/></svg>
                <span>Combos</span>
            </a>
        <?php endif; ?>

        <?php if (userIsAnyOf(['admin', 'customer'])): ?>
            <a href="<?= baseUrl('orders/browse') ?>" class="nav-item <?= ($currentPage ?? '') === 'browse' ? 'active' : '' ?>">
                <svg viewBox="0 0 24 24"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg>
                <span>Browse Restaurants</span>
            </a>
        <?php endif; ?>

        <a href="<?= baseUrl('orders') ?>" class="nav-item <?= ($currentPage ?? '') === 'orders' ? 'active' : '' ?>">
            <svg viewBox="0 0 24 24"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 3c1.93 0 3.5 1.57 3.5 3.5S13.93 13 12 13s-3.5-1.57-3.5-3.5S10.07 6 12 6zm7 13H5v-.23c0-.62.28-1.2.76-1.58C7.47 15.82 9.64 15 12 15s4.53.82 6.24 2.19c.48.38.76.97.76 1.58V19z"/></svg>
            <span><?= userIsDriver() ? 'My Deliveries' : (userIsCustomer() ? 'My Orders' : 'Orders') ?></span>
        </a>

        <?php if (userIsAdmin()): ?>
            <div class="nav-section-title">Administration</div>

            <a href="<?= baseUrl('reports') ?>" class="nav-item <?= ($currentPage ?? '') === 'reports' ? 'active' : '' ?>">
                <svg viewBox="0 0 24 24"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 14H7v-2h5v2zm3-4H7v-2h8v2zm0-4H7V7h8v2z"/></svg>
                <span>Reports</span>
            </a>

            <a href="<?= baseUrl('users/pending') ?>" class="nav-item <?= ($currentPage ?? '') === 'pending-users' ? 'active' : '' ?>">
                <svg viewBox="0 0 24 24"><path d="M12 4V1L8 5l4 4V6c3.31 0 6 2.69 6 6 0 1.01-.25 1.97-.7 2.8l1.46 1.46C19.54 15.03 20 13.57 20 12c0-4.42-3.58-8-8-8zm0 14c-3.31 0-6-2.69-6-6 0-1.01.25-1.97.7-2.8L5.24 7.74C4.46 8.97 4 10.43 4 12c0 4.42 3.58 8 8 8v3l4-4-4-4v3z"/></svg>
                <span>Pending Users</span>
            </a>
        <?php endif; ?>
    </div>

    <div class="nav-footer">
        <?php $navAvatar = avatarUrl((int) (currentUserId() ?? 0)); ?>
        <a href="<?= baseUrl('profile') ?>" class="nav-user" style="text-decoration: none; color: inherit;" title="Profile settings">
            <div class="nav-user-avatar <?= $navAvatar ? 'has-photo' : '' ?>">
                <?php if ($navAvatar): ?>
                    <img src="<?= e($navAvatar) ?>" alt="">
                <?php else: ?>
                    <?= strtoupper(substr($_SESSION['username'] ?? 'U', 0, 1)) ?>
                <?php endif; ?>
            </div>
            <div class="nav-user-info">
                <div class="nav-user-name"><?= e($_SESSION['username'] ?? 'User') ?></div>
                <div class="nav-user-role"><?= e($_SESSION['role'] ?? 'user') ?></div>
            </div>
        </a>
        <a href="<?= baseUrl('logout') ?>" class="btn btn-outline btn-block btn-sm">
            <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M17 7l-1.41 1.41L18.17 11H8v2h10.17l-2.58 2.58L17 17l5-5zM4 5h8V3H4c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h8v-2H4V5z"/></svg>
            Logout
        </a>
    </div>
</nav>

<div class="nav-overlay" id="navOverlay"></div>
