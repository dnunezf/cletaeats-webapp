...
class CustomerController
{
    ...
    public function index(): void
    {
        ...
        $customers = $search !== '' ? $this->customerService->search($search) : $this->customerService->getAll();
    ...
    public function update(): void
    {
        csrfCheck();
        $id = (int) ($_POST['id'] ?? 0);
        if ($id <= 0) { redirect('customers'); return; }
        $data = $this->extractFormData();
    ...
    public function delete(): void
    {
        ...
        $id = (int) ($_POST['id'] ?? 0);
        if ($id <= 0) {
            if (isAjax()) { jsonResponse(['success' => false, 'message' => 'Invalid customer ID.'], 400); }
            redirect('customers');
            return;
        }
        $this->customerService->delete($id);
        if (isAjax()) { jsonResponse(['success' => true, 'message' => 'Customer deleted successfully.']); return; }
